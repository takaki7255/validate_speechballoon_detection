...
// New function that returns panels with bounding box information
std::vector<Panel> Framedetect::frame_detect_with_bbox(cv::Mat &src_page){
    std::vector<Panel> panels_with_bbox;
    
    cv::Size img_size(src_page.cols, src_page.rows);
    cv::Mat zeros = cv::Mat::zeros(img_size, CV_8UC1);
    cv::Mat color_page = src_page;
    cv::cvtColor(color_page, color_page, cv::COLOR_GRAY2BGR);
    cv::Mat gray_img;
    if (src_page.type() != CV_8UC1) {
        cv::cvtColor(src_page, gray_img, cv::COLOR_BGR2GRAY);
    }else{
        gray_img = src_page;
    }
    ...
    cv::Mat binForSpeechBalloon_img;
    threshold(gray_img.clone(), binForSpeechBalloon_img, 230, 255, cv::THRESH_BINARY);
    cv::erode(binForSpeechBalloon_img, binForSpeechBalloon_img, cv::Mat(),cv::Point(-1,-1), 1);
    cv::dilate(binForSpeechBalloon_img, binForSpeechBalloon_img, cv::Mat(),cv::Point(-1,-1), 1);
    
    std::vector<cv::Vec4i> hierarchy2;
    std::vector<std::vector<cv::Point> > hukidashi_contours;
    cv::findContours(binForSpeechBalloon_img, hukidashi_contours, hierarchy2, cv::RETR_LIST, cv::CHAIN_APPROX_NONE);
    
    cv::Mat gaussian_img;
    cv::GaussianBlur(gray_img, gaussian_img, cv::Size(3,3), 0);
    extractSpeechBalloon(hukidashi_contours,hierarchy2, gaussian_img);
    
    cv::Mat inverse_bin_img;
    threshold(gaussian_img, inverse_bin_img, 210, 255, cv::THRESH_BINARY_INV);
    findFrameExistenceArea(inverse_bin_img);
    ...
    std::vector<cv::Point2f> lines;
    std::vector<cv::Point2f> lines2;
    HoughLines(canny_img, lines, 1, M_PI/180.0, 50);
    HoughLines(canny_img, lines2, 1, M_PI/360.0, 50);
    ...
    cv::Mat and_img = cv::Mat::zeros(img_size, CV_8UC1);
    bitwise_and(inverse_bin_img, lines_img, and_img);
    ...
    findContours(complement_and_img, contours3, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
    
    for (int i=0; i<contours3.size(); i++) {
        cv::Rect tmp_bounding_box = cv::boundingRect (contours3[i]);
        if(judgeAreaOfBoundingBox(tmp_bounding_box, complement_and_img.cols*complement_and_img.rows) == true){
            bounding_boxes.push_back(tmp_bounding_box);
        }
    }
    ...
    for(int i=0; i<contours3.size(); i++){
        ...
        // Store panel with bbox
        Panel panel;
        panel.img = cut_img.clone();
        panel.bbox = brect;
        panels_with_bbox.push_back(panel);
    }
    
    return panels_with_bbox;
}
