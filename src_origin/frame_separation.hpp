...
// Structure to hold panel image and its bounding box (position in page)
struct Panel {
    cv::Mat img;    // Panel image
    cv::Rect bbox;  // Position in page coordinates
};
...
{
public:
    std::vector<cv::Mat> frame_detect(cv::Mat &src_page);//コマ抽出関数 (original)
    std::vector<Panel> frame_detect_with_bbox(cv::Mat &src_page);//コマ抽出関数 (with bbox)
private:
